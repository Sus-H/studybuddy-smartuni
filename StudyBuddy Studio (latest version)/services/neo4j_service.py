...
class Neo4jService:
    ...
    def get_graph_statistics(self) -> Dict[str, int]:
        ...
        try:
            with self.driver.session() as session:
                ...
                stats['relations'] = session.run("MATCH (n1)-[r]->(n2) WHERE (n1:Kurs OR n1:Koncept OR n1:Student) AND (n2:Kurs OR n2:Koncept OR n2:Student) RETURN count(r) as count").single()['count']
    ...
    def get_existing_graph_as_json(self) -> str:
        ...
        try:
            with self.driver.session() as session:
                # Hämta alla noder - endast Kurs, Koncept och Student för StudyBuddy
                nodes_query = """
                MATCH (n)
                WHERE n:Kurs OR n:Koncept OR n:Student
                RETURN collect({
                    id: elementId(n),
                    labels: labels(n),
                ...
                rels_query = """
                MATCH (n1)-[r]->(n2)
                WHERE (n1:Kurs OR n1:Koncept OR n1:Student) AND (n2:Kurs OR n2:Koncept OR n2:Student)
                WITH n1, n2, r,
                     CASE 
                ...
                RETURN collect({
                    ...
                    to: to_id,
                    type: type(r),
                    from_label: head(labels(n1)),
                    to_label: head(labels(n2))
                }) as relationships
                """
                rels_result = session.run(rels_query).single()
                
            # Hjälpfunktion för att konvertera värden till JSON-serialiserbara typer
            def convert_value(val):
                if hasattr(val, 'isoformat'):  # DateTime-objekt
            ...
            # Konvertera Neo4j-objekt till Python-dict för JSON-serialisering
            nodes = []
            if nodes_result and nodes_result["nodes"]:
                for node in nodes_result["nodes"]:
                    props = {}
                    for key, value in node.get("properties", {}).items():
                        props[key] = convert_value(value)
            ...
            relationships = []
            if rels_result and rels_result["relationships"]:
            ...
            st.error(f"Fel vid hämtning av graf: {str(e)}")
