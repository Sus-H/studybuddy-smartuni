...
def render_concept_learning_ui(recommendation: Dict, knowledge_profile: Dict, course_code: str = None):
    ...
    if not recommendation or not recommendation.get('recommended_concept'):
        ...
        if course_code:
            # Hämta ENDAST koncept som faktiskt är i kursen (inte prerequisites)
            course_concepts_details = get_course_concepts_details(course_code)
            low_mastery_concepts = [c for c in course_concepts_details 
...
def render_course_graph_with_prerequisites(course_code: str):
    ...
    with st.session_state.neo4j_service.driver.session() as session:
        # Steg 1: Hämta bara kursens koncept först
        course_result = session.run("""
            MATCH (k:Kurs {kurskod: $kurskod})
            OPTIONAL MATCH (k)-[:INNEHÅLLER]->(c:Koncept)
            WITH k, c, COALESCE(c.mastery_score, 0.0) as mastery
            RETURN k as course,
                   COLLECT(DISTINCT {
                       node: c, 
                       namn: c.namn,
        ...
        course_record = course_result.single()
        if not course_record or not course_record['course']:
            st.warning("Ingen data hittades för kursen")
            return
            
        course = course_record['course']
        course_concepts_data = course_record['course_concepts']
        course_concepts = [c['node'] for c in course_concepts_data if c['node'] is not None]
        
        # Om inga koncept finns
        if not course_concepts:
            st.info(f"Kursen {course_code} har inga koncept i databasen ännu.")
            return
        
        # Steg 2: Hämta ENDAST direkta prerequisites (1 steg)
        prereq_result = session.run("""
            MATCH (k:Kurs {kurskod: $kurskod})-[:INNEHÅLLER]->(c:Koncept)
            OPTIONAL MATCH (c)-[:FÖRUTSÄTTER]->(prereq:Koncept)
            WHERE NOT (k)-[:INNEHÅLLER]->(prereq)
            WITH DISTINCT prereq, COALESCE(prereq.mastery_score, 0.0) as mastery
            WHERE prereq IS NOT NULL
            RETURN COLLECT({
                node: prereq,
                namn: prereq.namn,
                mastery_score: mastery
            })[..30] as external_prereqs  // Begränsa till max 30 externa prerequisites
        """, kurskod=course_code)
        
        prereq_record = prereq_result.single()
        external_prereqs_data = prereq_record['external_prereqs'] if prereq_record else []
        external_prereqs = [p['node'] for p in external_prereqs_data]
        
        # Steg 3: Hämta relationer mellan de koncept vi har
        all_concepts = course_concepts + external_prereqs
        all_concept_names = [c.get('namn') for c in all_concepts if c.get('namn')]
        
        if len(all_concept_names) > 0:
            rel_result = session.run("""
                UNWIND $concept_names as name1
                UNWIND $concept_names as name2
                MATCH (c1:Koncept {namn: name1})-[:FÖRUTSÄTTER]->(c2:Koncept {namn: name2})
                RETURN COLLECT(DISTINCT {
                    from: c1.namn,
                    to: c2.namn
                }) as prerequisites
            """, concept_names=all_concept_names[:100])  # Begränsa antal koncept
            
            rel_record = rel_result.single()
            prerequisites = rel_record['prerequisites'] if rel_record else []
        else:
            prerequisites = []
        
        # Bygg mastery map från data vi hämtat
        mastery_map = {}
        for c in course_concepts_data:
            if c['namn']:
                mastery_map[c['namn']] = c['mastery_score']
        for p in external_prereqs_data:
            if p['namn']:
                mastery_map[p['namn']] = p['mastery_score']
        
        # Lägg till mastery scores till prerequisites
        for prereq in prerequisites:
            prereq['from_mastery'] = mastery_map.get(prereq['from'], 0.0)
            prereq['to_mastery'] = mastery_map.get(prereq['to'], 0.0)
