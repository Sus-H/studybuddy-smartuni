...
from utils.session import init_session_state, lazy_init_llm_service
...
def get_llm_service():
    """Hämtar LLM service med konsekvent felhantering"""
    llm_service = lazy_init_llm_service()
    if not llm_service:
        st.error("Kunde inte ansluta till AI-tjänsten. Kontrollera din API-nyckel i inställningar.")
        return None
    return llm_service
...
def render():
    ...
    with col2:
        ...
        # Kolla om svårighetsnivå har ändrats
        previous_difficulty = st.session_state.get('selected_difficulty', difficulty_options[0])
        
        selected_difficulty = st.selectbox(
            "Välj svårighetsnivå:",
            difficulty_options,
            index=0,
            help="I en fullskalig version skulle AI alltid välja det optimala konceptet baserat på din kunskapsprofil",
            key="difficulty_selector"
        )
        
        # Om svårighetsnivå har ändrats, hitta nytt koncept
        if selected_difficulty != previous_difficulty:
    ...
    if 'current_training' not in st.session_state and 'smart_training_initialized' in st.session_state:
        with st.spinner("AI analyserar din kunskapsprofil..."):
            # Hämta vald svårighetsnivå
            selected_difficulty = st.session_state.get('selected_difficulty', "AI väljer optimalt koncept")
            
            # Skicka svårighetsnivå om det inte är AI-val
            difficulty_param = None if selected_difficulty == "AI väljer optimalt" else selected_difficulty
            
            optimal_concept, score_details = find_optimal_concept(difficulty_param)
            if optimal_concept:
    ...
    try:
        with st.session_state.neo4j_service.driver.session() as session:
            ...
            result = session.run("""
                ...
                LIMIT 10
                UNION
                MATCH (c:Koncept {namn: $namn})-[:FÖRUTSÄTTER]-(other:Koncept)
                RETURN DISTINCT other.namn as namn
                LIMIT 10
            """, namn=concept_name)
...
def find_optimal_concept(difficulty_filter: str = None) -> Tuple[Optional[Dict], Dict]:
    ...
    # Filtrera koncept baserat på svårighetsnivå om specificerat
    if difficulty_filter:
        if difficulty_filter == "Grundläggande":
            # Filtrera koncept med låg mastery och inga/få prerequisites
            concepts = [c for c in concepts if c.get('mastery_score', 0) < 0.3 and len(c.get('prerequisites', [])) <= 1]
        elif difficulty_filter == "Medelnivå":
            # Filtrera koncept med medel mastery
            concepts = [c for c in concepts if 0.2 <= c.get('mastery_score', 0) <= 0.7]
        elif difficulty_filter == "Avancerat":
            # Filtrera koncept med högre mastery eller många prerequisites
            concepts = [c for c in concepts if c.get('mastery_score', 0) > 0.5 or len(c.get('prerequisites', [])) > 2]
...
def calculate_concept_score(concept: Dict) -> Tuple[float, Dict]:
    ...
    # Kommenterat bort tills FÖRVÄXLAS_MED relationer finns i databasen
    # För att undvika warnings sätter vi discrimination_bonus till 0
    discrimination_bonus = 0.0
    
    # När FÖRVÄXLAS_MED finns i databasen, avkommentera följande:
    # try:
    #     with st.session_state.neo4j_service.driver.session() as session:
    #         result = session.run("""
...
def show_guided_learning(concept: Dict):
    ...
    with col2:
        ...
        mastery = concept.get('mastery_score', 0)
        st.metric("Mastery", f"{mastery:.0%}", label_visibility="collapsed")
...
def get_related_mastered_concepts(concept_name: str) -> List[str]:
    """Hämtar relaterade koncept som studenten behärskar bra"""
    try:
        with st.session_state.neo4j_service.driver.session() as session:
            result = session.run("""
                MATCH (c:Koncept {namn: $namn})-[:FÖRUTSÄTTER|FÖRUTSÄTTS_AV*..2]-(related:Koncept)
                WHERE related.mastery_score > 0.6
                RETURN DISTINCT related.namn as namn
                LIMIT 3
            """, namn=concept_name)
...
def analyze_student_answer(concept: Dict, task: Dict, student_answer: str) -> Dict:
    """Analyserar studentens svar med LLM"""
    
    try:
        # Hämta LLM service med lazy init
        llm_service = lazy_init_llm_service()
        if not llm_service:
            return {
                'score': 0,
                'feedback': 'Kunde inte ansluta till AI-tjänsten.',
                'strengths': [],
                'weaknesses': []
            }
        
        # Hämta relaterade koncept som studenten kan bra
        related_concepts = get_related_mastered_concepts(concept['namn'])
        related_text = ""
        if related_concepts:
            related_text = f"\nKONCEPT STUDENTEN KAN BRA (använd för att förklara): {', '.join(related_concepts)}"
        
        # Bygg prompt med full kontext
        prompt = f"""
Du är en vänlig och uppmuntrande lärare som ger feedback till en student.

KONCEPT SOM TRÄNAS: {concept['namn']}
{related_text}

UPPGIFT:
{task.get('task', 'Ingen uppgift tillgänglig')}

STUDENTENS SVAR:
{student_answer}

FACIT (använd som referens men nämn inte "facit" för studenten):
{task.get('solution', 'Ingen lösning tillgänglig')}

Ge personlig och hjälpsam feedback direkt till studenten (använd "du"):

1. Börja positivt - lyft fram något bra i svaret
2. Ge konkreta tips på vad som saknas eller kan förbättras
3. Om möjligt, relatera till koncept studenten redan kan bra
4. Ge ett litet kodexempel eller förklaring som hjälper studenten vidare
5. Avsluta med uppmuntran och nästa steg

Var vänlig och pedagogisk. Förklara svåra koncept enkelt. Ge konkreta exempel.
Skriv som om du pratar direkt med studenten. Max 3-4 stycken.
"""
        
        response = llm_service.query(prompt)
        
        # Returnera feedback direkt
        return {
            'feedback': response,
            'score': 0,  # Behövs inte längre men behåller för kompatibilitet
            'strengths': [],
            'weaknesses': []
        }
        ...
        return {
            'score': 0,
            'feedback': f'Ett fel uppstod vid analys: {str(e)}',
            'strengths': [],
            'weaknesses': []
        }
...
def show_advanced_mode(concept: Dict):
    ...
    # Generera avancerad uppgift med caching per koncept
    cache_key = f"advanced_task_{concept['namn']}"
    if cache_key not in st.session_state:
        with st.spinner("Genererar avancerad uppgift..."):
            st.session_state[cache_key] = generate_advanced_task(concept)
        # Rensa feedback och chatt när ny uppgift genereras
        if 'show_feedback' in st.session_state:
            del st.session_state.show_feedback
        if 'last_feedback' in st.session_state:
            del st.session_state.last_feedback
        if 'show_hint' in st.session_state:
            del st.session_state.show_hint
        if 'chat_history' in st.session_state:
            del st.session_state.chat_history
    
    task = st.session_state[cache_key]
    
    # Visa uppgift
    if task.get('task') and len(task['task'].strip()) > 10:  # Kolla att uppgiften har rimlig längd
        st.markdown("**Uppgift:**")
        st.markdown(task['task'])  # Visa uppgiften med markdown-rendering
    else:
        # Om uppgiften inte genererats korrekt, generera om
        st.warning("Uppgiften genererades inte korrekt. Genererar ny uppgift...")
        if cache_key in st.session_state:
            del st.session_state[cache_key]
        st.rerun()
    
    # Lösningsområde
    solution = st.text_area("Din lösning:", height=200, placeholder="Skriv din lösning här...")
    
    col1, col2 = st.columns(2)
    
    with col1:
        if st.button("Rätta", type="primary"):
            if solution.strip():
                # Analysera studentens svar med LLM
                feedback = analyze_student_answer(concept, task, solution)
                st.session_state.last_feedback = feedback
                st.session_state.show_feedback = True
            else:
                st.warning("Skriv din lösning först innan du klickar på Rätta.")
    
    with col2:
        if st.button("Visa ledtråd"):
            if 'show_hint' not in st.session_state:
                st.session_state.show_hint = True
            else:
                st.session_state.show_hint = not st.session_state.show_hint
    
    # Visa feedback om rättning har gjorts
    if st.session_state.get('show_feedback', False) and st.session_state.get('last_feedback'):
        feedback = st.session_state.last_feedback
        
        # Visa bara LLM:s feedback utan poäng eller statusmeddelanden
        st.markdown(feedback['feedback'])
        
        # Lägg till chattmöjlighet
        st.markdown("---")
        ...
        # Chattinput
        follow_up = st.text_input("Har du någon följdfråga?", key="follow_up_question")
        
        if follow_up and st.button("Skicka fråga"):
            ...
            if llm:
                chat_prompt = f"""
Du är en hjälpsam lärare som fortsätter hjälpa studenten med konceptet {concept['namn']}.

KONTEXT:
- Koncept: {concept['namn']}
- Uppgift: {task.get('task', '')}
- Studentens ursprungliga svar: {solution}
- Din tidigare feedback: {feedback['feedback']}

STUDENTENS FRÅGA:
{follow_up}

Ge ett pedagogiskt och hjälpsamt svar. Använd exempel när det är lämpligt.
"""
                
                response = llm.query(chat_prompt)
        ...
        for message in st.session_state.chat_history:
            if message["role"] == "user":
                ...
                st.markdown(f"**Lärare:** {message['content']}")
    
    # Visa extra ledtråd om begärt
    if st.session_state.get('show_hint', False) and task.get('hint'):
        with st.expander("Extra ledtråd", expanded=True):
...
Ge DIREKT och KONCIST:
1. Uppgiftsbeskrivning: (En konkret uppgift, max 3 meningar)
2. Ledtråd: (En kort vägledning)
3. Lösning: (Huvudpunkterna i lösningen)

VIKTIGT: Var konkret och kortfattad."""
    ...
    try:
        ...
        # Använd completion direkt för bättre kontroll
        response = completion(
            model=llm.model,
            messages=[
                {"role": "system", "content": "Du är en expert på att skapa programmeringsuppgifter. Svara ALLTID kort och konkret."},
                {"role": "user", "content": prompt}
            ],
            temperature=0.5,  # Lägre temperatur för snabbare svar
            max_tokens=300,  # Begränsa längden för snabbare respons
            api_key=llm.api_key,
        ...
        # Parsing av svaret med bättre hantering
        import re
        ...
        # Försök olika mönster för att hitta uppgiften
        # Mönster 1: "1. Uppgiftsbeskrivning: [text]"
        match1 = re.search(r'1\.\s*(?:Uppgiftsbeskrivning:?)?\s*(.+?)(?=\n2\.|$)', response, re.DOTALL)
        if match1:
            task = match1.group(1).strip()
        
        # Om mönster 1 misslyckades, försök mönster 2: "Uppgiftsbeskrivning:\n[text]"
        if not task or task == "Uppgiftsbeskrivning":
            match2 = re.search(r'Uppgiftsbeskrivning:\s*\n(.+?)(?=\n\n|(?:\n)?2\.|Ledtråd:|$)', response, re.DOTALL)
            if match2:
                task = match2.group(1).strip()
        
        # Om fortfarande ingen uppgift, ta första stycket som inte är en rubrik
        if not task or task == "Uppgiftsbeskrivning":
            lines = response.split('\n')
            for i, line in enumerate(lines):
                if line.strip() and not any(x in line for x in ['1.', '2.', '3.', 'Uppgiftsbeskrivning:', 'Ledtråd:', 'Lösning:']):
                    task = line.strip()
                    # Ta med efterföljande rader tills nästa rubrik
                    for j in range(i+1, len(lines)):
                        if any(x in lines[j] for x in ['2.', 'Ledtråd:', '3.', 'Lösning:']):
                            break
                        if lines[j].strip():
                            task += " " + lines[j].strip()
                    break
        ...
        # Extrahera ledtråd
        hint_match = re.search(r'(?:2\.\s*)?(?:Ledtråd|En ledtråd):?\s*(.+?)(?=\n3\.|Lösning:|$)', response, re.DOTALL)
        if hint_match:
            hint = hint_match.group(1).strip()
        
        # Extrahera lösning  
        solution_match = re.search(r'(?:3\.\s*)?(?:Fullständig lösning|Lösning):?\s*(.+?)$', response, re.DOTALL)
        if solution_match:
            solution = solution_match.group(1).strip()
        
        # Fallback värden om parsing misslyckades
        if not task:
            task = f"Implementera en avancerad lösning som använder {concept['namn']}"
        if not hint:
            hint = "Tänk på de grundläggande principerna och hur de kan tillämpas"
        if not solution:
            solution = "Lösningen kommer att visa hur konceptet tillämpas i praktiken"
...
def get_confused_concepts(concept_name: str) -> List[str]:
    ...
    # Returnerar tom lista tills FÖRVÄXLAS_MED finns i databasen
    return []
    
    # När FÖRVÄXLAS_MED finns i databasen, ersätt med:
    # try:
    #     with st.session_state.neo4j_service.driver.session() as session:
    #         result = session.run("""
    #             MATCH (c:Koncept {namn: $namn})-[r:FÖRVÄXLAS_MED]-(other:Koncept)
...
def show_quick_test_section(concept: Dict):
    ...
    st.caption("⏱️ Snabbtest använder förenklad evaluering för snabbare respons")
    ...
    if 'test_questions' not in st.session_state:
        with st.spinner("Förbereder snabbtest..."):
...
def generate_misconceptions(concept: Dict) -> str:
    ...
    if confused_with:
        # Vi har data om vad som förväxlas
