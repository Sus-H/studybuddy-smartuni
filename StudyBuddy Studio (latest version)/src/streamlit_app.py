...
def main():
    ...
    # Lazy loading - importera och rendera
    with tab1:
        render_courses_tab()
                ...
                for year in sorted(courses_df['year'].unique()):
                    ...
                    for period in sorted(year_courses['period_num'].unique()):
                        if period > 0:  # Hoppa över kurser utan period
                            ...
                            for idx, course in period_courses.iterrows():
                                # Skapa unik nyckel för varje kurs (inkludera index för att undvika dubbletter)
                                course_key = f"tab1_select_{course['courseCode']}_{year}_{period}_{idx}"
                                ...
                                with col2:
                                    ...
                                    with st.expander(
                                        ...
                                        show_course_details(course, year, period, idx)
            else:
...
def show_course_details(course, year, period, idx):
    ...
    with col2:
        if st.button(f"Lägg till i graf", key=f"build_{course['courseCode']}_{year}_{period}_{idx}"):
...
def build_program_graph_with_selection(program_code, courses_df):
    ...
    for _, course in courses_df.iterrows():
        if course['rule'] == 'O':
            ...
            # Kolla om valbar kurs är vald (kolla alla keys som matchar kurskoden)
            course_selected = False
            for key in st.session_state.keys():
                if key.startswith(f"tab1_select_{course['courseCode']}_") and st.session_state.get(key, False):
